...
class CarteFizica(Carte):
    def __init__(self, titlu, autor, locatia, status, editura):
...
class Notificare:
    ...
    def trimite_notificare(self):
        ...
        print("Se va trimite un email pt ca data de retur a fost depasita.")
        server.sendmail(self.adresa_from, self.adresa_to, mesaj)
        server.quit()

if __name__ == '__main__':

    carte_fizica = CarteFizica(" Luceafarul ", " Mihai Eminescu ", "Raft 2", "neimprumutata", "Nemira")
    ...
    if bibliotecar.verifica_disponibilitate(carte_fizica):
        ...
        if carte_fizica.data_retur > datetime.now() - timedelta(weeks = 5):
            notificare = Notificare('local@telacad.ro','dest@telacad.ro',
                                    "Alerta returnare carte", "Va rugam sa returnati cartea" +
                                    carte_fizica.autor + carte_fizica.titlu)
